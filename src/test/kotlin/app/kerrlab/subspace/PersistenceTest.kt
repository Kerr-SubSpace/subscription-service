...
import app.kerrlab.subspace.service.persistence.repository.SubscriptionRepository
...
) : FeatureSpec(
    {
        ...
        feature("persistence context") {
            ...
            scenario("should allow saving new entities") {
                val subscription = SubscriptionModel("Spongify")
            ...
            scenario("should find entity by id") {
                val subscription = SubscriptionModel("Wamazon")
                subscriptionRepository.save(subscription)
            ...
            scenario("should find all entities") {
                val subscription = SubscriptionModel("Nerdflix")
                subscriptionRepository.save(subscription)
                subscription.id?.shouldNotBeNull()
                logger.info("Retrieving subscription with id {}", subscription.id)
            ...
            scenario("should find by name") {
                val subscription = SubscriptionModel("CornPub")
                subscriptionRepository.save(subscription)
            ...
            scenario("should allow updating entities") {
                val subscription = SubscriptionModel("WeTube Premium")
                subscriptionRepository.save(subscription)
            ...
            scenario("should allow deleting entities") {
                val subscription = SubscriptionModel("Gluggle One")
                subscriptionRepository.save(subscription)
